        ...
        for time_avg in ['Seasonal','Monthly']:
            ...
            for iwindow in range(len(start_time_list)):
                ...
                for key in center_arrow_dict.keys():
                    ...
                    # append to the list
                    xL.append(x - center_arrow_offset)
                    yL.append(y)
                    dxL.append(0)
                    dyL.append(1)
                    xR.append(x)
                    ...
                    dxR.append(0)
                    dyR.append(1)
                    xS.append(x + center_arrow_offset)
                    yS.append(y)
                    dxS.append(0)
                    dyS.append(1)
                ...
                for i in range(len(storage)):
                    ...
                    head_width = 2*width
                    head_length = 1.5*width
                    ax.arrow(xS[i], yS[i], dxS[i]*length, dyS[i]*length, width = width, head_width=head_width, head_length=head_width, length_includes_head=True, color=storage_color, edgecolor=None, alpha=1)
                ...
                plt.text(legend_loc[0]+1.5*length,legend_loc[1]+leg_offset,'%0.0f Mg/d Reaction' % arrow_scale_Mgd,fontsize=fontsize, va='center')
                leg_offset = leg_offset - legend_yspace
                # ... storage arrow
