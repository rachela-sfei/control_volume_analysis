...
# arrow scale length (m)
arrow_scale_length = 5000

# arrow scale flux (Mg/d)
arrow_scale_flux_transport = 20
arrow_scale_flux_loading = 10
arrow_scale_flux_reaction = 5

# arrow scale (m per Mg/d)
arrow_scale_transport = arrow_scale_length / arrow_scale_flux_transport
arrow_scale_loading = arrow_scale_length / arrow_scale_flux_loading
arrow_scale_reaction = arrow_scale_length / arrow_scale_flux_reaction

# what fraction of the arrow length to make the width
arrow_width = 0.1

# offset between loading and reaction arrows in meters
arrow_offset = 3000
...
# indices of control volumes in the shapefile that we want to plot
...
# for example arrow 0 is located at the N end of LSB, points into LSB, and we get its
# magnitude from the LSB flux into the N (note in balance tables fluxes are given w.r.t.
# the directions pointing into the control volume)
flux_arrow_dict = {}
...
# axis window
axis_window = (519062.33981440606, 614035.4169852139, 4134370.722411021, 4236807.203694713)
...
# load the balance table and average over the specified time period
...
for iwindow in range(len(start_time_list)):
    ...
    # get the loading and reactive loss arrow info
    xL = []
    yL = []
    dxL = []
    dyL = []
    ...
    for key in center_arrow_dict.keys():
        ...
        # get the loading and the reactions
    ...
    shp.iloc[iplot].boundary.plot(ax=ax, color = 'b')
    for i in range(len(loading)):
        length = loading[i]*arrow_scale_loading
        width  = length*arrow_width
        ax.arrow(xL[i] - arrow_offset/2, yL[i], dxL[i]*length, dyL[i]*length, color='black', width = width, head_width=3*width, head_length=3*width)
    for i in range(len(reaction)):
        length = reaction[i]*arrow_scale_reaction
        width  = length*arrow_width
        ax.arrow(xR[i] - arrow_offset/2, yR[i], dxR[i]*length, dyR[i]*length, color='red', width = width, head_width=3*width, head_length=3*width)
    for i in range(len(transport)):
        length = transport[i]*arrow_scale_transport
        width  = length*arrow_width
        ax.arrow(xT[i], yT[i], dxT[i]*length, dyT[i]*length, color='green', width = width, head_width=3*width, head_length=3*width)
    
    # fontsize
    fontsize=16
    
    # add a legend
    length = arrow_scale_length
    width = arrow_scale_length*arrow_width
    ax.arrow(legend_loc[0],legend_loc[1],length,0,width=width, head_width=3*width, head_length=3*width,color='green')
    ax.arrow(legend_loc[0],legend_loc[1]-legend_yspace,length,0,width=width, head_width=3*width, head_length=3*width,color='black')
    ax.arrow(legend_loc[0],legend_loc[1]-2*legend_yspace,length,0,width=width, head_width=3*width, head_length=3*width,color='red')
    plt.text(legend_loc[0]+1.5*length,legend_loc[1],'%0.0f Mg/d Transport' % arrow_scale_flux_transport, fontsize=fontsize, va='center')
    plt.text(legend_loc[0]+1.5*length,legend_loc[1]-legend_yspace,'%0.0f Mg/d Loading' % arrow_scale_flux_loading,fontsize=fontsize, va='center')
    plt.text(legend_loc[0]+1.5*length,legend_loc[1]-2*legend_yspace,'%0.0f Mg/d Reaction' % arrow_scale_flux_reaction,fontsize=fontsize, va='center')
