...
# y axis min/max (code will automatically scale y axis figure by figure, if want to match between 2 figures need this)
ymax_override = {}
ymax_override = {'DIN' : {'Area' : {'fall' : 410, 'winter' : 2040, 'spring' : 700, 'summer' : 170}},
                 'TN' : {'Area' : {'fall' : 410, 'winter' : 2040, 'spring' : 700, 'summer' : 170}}}
    ...
    for norm in ['Total','Area','Volume']:
        ...
        for tperiod in ['annual','fall','winter','spring','summer']:
            ...
            if separate_source_sink_flag:
                figure_name = '%s%s_Subembayment_Mass_Budget_Bar_Plot_Stacked_Source_Sink_%s_%s.png' % (run_ID_prefix, param, tsuf, norm)
            else:
            ...
            # override ymax if value is available in ymax_override (for example, to match TN and DIN axes)
            if param in ymax_override.keys():
                if norm in ymax_override[param].keys():
                    if tperiod in ymax_override[param][norm].keys(): 
                        ymax = ymax_override[param][norm][tperiod]
